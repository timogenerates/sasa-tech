...
export async function consumeCreditForUserId(userId: string): Promise<UsageResult> {
  const { data: profile, error } = await supabaseAdmin
    ...
    .single();
  if (error || !profile) {
    console.error("[usage] read profile", error);
  ...
  if (profile.tier === "monthly") {
    return { ok: true, tier: "monthly", dailyUsed: 0, dailyLimit: -1, promptsRemaining: -1 };
  }
